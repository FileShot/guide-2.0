...
// These modules use require('electron') internally (agenticChat.js, llmEngine.js).
// Since we ARE Electron, no shim is needed — they get the real module.
...
const { DEFAULT_SYSTEM_PREAMBLE, DEFAULT_COMPACT_PREAMBLE, DEFAULT_CHAT_PREAMBLE } = require('./constants');
...
const agenticChat = require('./agenticChat');
...
// ─── Build context object (for agenticChat.register) ─────────────────
const ctx = {
  llmEngine,
  mcpToolServer,
  memoryStore,
...
// Register agentic chat handlers (ai-chat, agent-pause, agent-resume)
// These call ipcMain.handle() internally — using the REAL ipcMain since we ARE Electron
...
ipcMain.handle('api-fetch', async (_event, url, options) => {
  ...
  try {
    ...
    if (p === '/api/project/open' && method === 'POST') {
      ...
      if (!fs.existsSync(resolved)) return { _status: 404, error: 'Directory not found' };
    ...
    if (p === '/api/templates/create' && method === 'POST') {
      ...
      return { success: true, projectDir, projectName: safeName, filesCreated: createdFiles };
    }
